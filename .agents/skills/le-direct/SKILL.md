...
1. detecte model/effort ativos conforme `RUNTIME-IDENTITY.md`;
...
Conduza o copilot como:

```text
intenção
→ editorial
→ estilo como gramática + visual por section
...
O copilot-driven enrichment roda antes de `CONCEPTION READY` quando material, converge para seleção compacta ou `NONE` e não consome os ciclos 0..3 do usuário. Esses ciclos pertencem exclusivamente ao checkpoint user-driven.
...
Ao atingir readiness, use o checkpoint user-driven/finalization, rode gate `PROMPT_COMPILE`, gere prompt imutável e calcule a recomendação de `PROMPT_EXECUTION` pela complexidade do prompt real.

Em `PROMPT READY`, siga `POST-PROMPT-DECISION.md`:
...
`Executar prompt` exige novo runtime detection + gate `PROMPT_EXECUTION` antes de qualquer mutação. Ao terminar, mostre `Refine | Finalizar`; nunca marque a interação como concluída automaticamente.
